...
./ehs -b -c $sndConnStr -m 'bWVzc2FnZQ=='

# send a single message with repetition
./ehs -c $sndConnStr -m 'message' -r 10

# send a single message with repetition and interval(in milliseconds) among the repetitions
./ehs -c $sndConnStr -m 'message' -r 10 -i 500

# send a single message with a property
./ehs -c $sndConnStr -m 'message2' -p "messageId:123123123"

# send a single message with multiple properties
./ehs -c $sndConnStr -m 'message2' -p "messageId:123123123;trackingId:123123123;businessLogicId:123123123;"

# send an amount of messages with batch of messages
./ehs -c $sndConnStr -m 'message' -bm -n 500000

# send a single message with repetition and interval(in milliseconds) among the repetitions
# with batch of messages
./ehs -c $sndConnStr -m 'message' -r 10 -i 500 -bm -n 500000

# replay messages from a file (receiver -c connString > messages-received.json)
./ehs -c $sndConnStr -rp -fp "/mypath/to/the/file/messages-received.json"

# replay messages from a file (receiver -c connString > messages-received.json)
# with repetition and interval among them.
./ehs -c $sndConnStr -r 10 -i 500 -rp -fp "/mypath/to/the/file/messages-received.json"
